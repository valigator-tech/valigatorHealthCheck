...
# Define a null output for quiet mode
if [ "$QUIET_MODE" = true ]; then
  # Redirect all output except the final summary to /dev/null
  exec 3>&1
...
check_sysctl() {
  ...
  if [ $? -ne 0 ]; then
    output "${RED}ERROR: Could not retrieve $param value. This may not be a Linux system or you don't have permission.${NC}"
    return 1
  fi
...
# Run all checks
output "${BLUE}Starting system health checks...${NC}"
